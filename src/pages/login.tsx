 ...
 async function formSubmit(event: React.SyntheticEvent){
  ...
  return (
    ...
    <form onSubmit={formSubmit}  className='relative h-[70vh] bg-black/75 w-[450px] mx-auto p-16 pt-16'>
      ...
      <button type="submit" className='w-full p-3 text-lg mt-4 font-semibold rounded-md bg-netflixRed outline-none'>Sign In</button>
      <p className='text-end flex-end'>New User? click <Link to="/register" className=" text-netflixRed hover:underline"> here </Link> to signUp </p>
      </section>
    </form>
    </>
  )
